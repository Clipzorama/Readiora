  ...
  return (
    <SectionReveal id="mission-control" className="relative z-10 mx-auto w-full max-w-[88rem] scroll-mt-24 px-3 py-16 sm:px-4 sm:py-20 lg:px-5 lg:py-28 xl:px-6">
      <div className="mx-auto max-w-4xl text-center">
        <div className="landing-card-surface mx-auto inline-flex items-center gap-2 rounded-full border border-border bg-card/80 px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-secondary">
      ...
      <div className="mt-10 sm:mt-12">
        <SpotlightCard
          ...
          className="landing-card-surface relative overflow-hidden rounded-[1.75rem] border border-strong-border/60 bg-card/90 p-3 shadow-2xl shadow-button/20 sm:p-4 lg:rounded-[2.125rem] lg:p-5"
        >
          <motion.div
            aria-hidden="true"
            animate={
          ...
          />
          <div className="landing-panel-surface relative overflow-hidden rounded-[1.35rem] border border-border bg-background/88">
            <motion.div
              aria-hidden="true"
              ...
              <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:justify-end">
                {statusPills.map((pill, index) => (
                  <motion.span
                    ...
                    className="landing-card-surface rounded-full border border-border bg-card/80 px-3 py-2 text-center text-[11px] font-semibold uppercase tracking-[0.14em] text-secondary"
                  >
                    {pill}
                  </motion.span>
                ))}
            ...
            <div className="grid gap-4 p-4 sm:p-5 lg:grid-cols-[minmax(0,0.92fr)_minmax(25rem,1.08fr)]">
              <div className="landing-card-surface min-w-0 overflow-hidden rounded-[1.35rem] border border-border bg-card/65">
                <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
                  <div className="flex min-w-0 items-center gap-3">
                    <div className="landing-soft-surface flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-border bg-background/75">
                ...
                <div className="space-y-3 p-4">
                  {activityFeed.map((item, index) => (
                    <motion.div
                      ...
                      className="landing-soft-surface group rounded-2xl border border-border bg-background/70 px-3 py-3 transition hover:border-strong-border hover:bg-background/90 sm:px-4"
                    >
              ...
              <div className="min-w-0 space-y-4">
                <div className="grid gap-3 sm:grid-cols-2">
                  {dashboardMetrics.map((metric, index) => {
                    ...
                    return (
                      <motion.div
                        key={metric.label}
                        className="landing-card-surface rounded-[1.25rem] border border-border bg-card/70 p-4 shadow-lg shadow-black/15"
                      >
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="landing-soft-surface flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-border bg-background/75">
                            <Icon className="h-5 w-5 text-secondary" />
                          </div>
                        </div>
                        <div className="landing-soft-surface mt-4 h-2 overflow-hidden rounded-full bg-background">
                          <motion.div
                ...
                <div className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
                  <div className="landing-card-surface rounded-[1.25rem] border border-border bg-card/70 p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-xs uppercase tracking-[0.18em] text-secondary">
                          Review Queue
                    ...
                    <div className="mt-4 space-y-2">
                      {reviewQueue.map((item, index) => (
                        <motion.div
                          ...
                          className="landing-soft-surface flex items-center justify-between gap-3 rounded-xl border border-border bg-background/70 px-3 py-2.5 text-sm"
                        >
                    ...
                    <div className="mt-4 space-y-3">
                      {weakTopics.map((topic, index) => (
                        <div key={topic.label}>
                          ...
                          <div className="landing-soft-surface mt-2 h-1.5 overflow-hidden rounded-full bg-background">
                            <motion.div
                              initial={prefersReducedMotion ? false : { width: 0 }}
                              whileInView={
                                prefersReducedMotion
